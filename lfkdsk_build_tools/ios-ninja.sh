#!/usr/bin/env bash
# ninja -C out/ios_debug
# ninja -C out/ios_debug_arm
# ninja -C out/ios_debug_sim
# ninja -C out/ios_dynamic_profile
# ninja -C out/ios_dynamic_profile_arm
# ninja -C out/ios_dynamic_release
# ninja -C out/ios_dynamic_release_arm
# ninja -C out/ios_fly_profile
# ninja -C out/ios_fly_profile_arm
# ninja -C out/ios_fly_release
# ninja -C out/ios_fly_release_arm
# ninja -C out/ios_flydynamic_debug
# ninja -C out/ios_flydynamic_debug_arm
# ninja -C out/ios_flydynamic_debug_sim
# ninja -C out/ios_flydynamic_dynamic_profile
# ninja -C out/ios_flydynamic_dynamic_profile_arm
# ninja -C out/ios_flydynamic_dynamic_release
# ninja -C out/ios_flydynamic_dynamic_release_arm
# ninja -C out/ios_profile
# ninja -C out/ios_profile_arm
# ninja -C out/ios_release
# ninja -C out/ios_release_arm


ninja -C out/ios_fly_debug_sim

ninja -C out/ios_flydynamic_debug_sim

ninja -C out/ios_fly_profile
ninja -C out/ios_fly_profile_arm

ninja -C out/ios_flydynamic_dynamic_profile_arm
ninja -C out/ios_flydynamic_dynamic_profile

ninja -C out/ios_fly_release
ninja -C out/ios_fly_release_arm

ninja -C out/ios_flydynamic_dynamic_release
ninja -C out/ios_flydynamic_dynamic_release_arm
