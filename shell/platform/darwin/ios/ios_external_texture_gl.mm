...
void IOSExternalTextureGL::CreateYUVTexturesFromPixelBuffer() {
    ...
    size_t height = CVPixelBufferGetHeight(buffer_ref_);
    {
        ...
        CVReturn err = CVOpenGLESTextureCacheCreateTextureFromImage(
                                                                    kCFAllocatorDefault, cache_ref_, buffer_ref_, nullptr, GL_TEXTURE_2D, GL_LUMINANCE,
                                                                    width,
    ...
    
    {
        ...
        CVReturn err = CVOpenGLESTextureCacheCreateTextureFromImage(
                                                                    ...
                                                                    width/2,
                                                                    height/2, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, 1,
                                                                    &uvTexture);
        if (err != noErr) {
...
sk_sp<SkImage> IOSExternalTextureGL::CreateImageFromRGBATexture(GrContext* context, const SkRect& bounds)
{
...
sk_sp<SkImage> IOSExternalTextureGL::CreateImageFromYUVTextures(GrContext* context, const SkRect& bounds)
{
    GrGLTextureInfo yTextureInfo = {CVOpenGLESTextureGetTarget(y_texture_ref_), CVOpenGLESTextureGetName(y_texture_ref_), GR_GL_LUMINANCE8};
    GrBackendTexture yBackendTexture(bounds.width(), bounds.height(), GrMipMapped::kNo, yTextureInfo);
    GrGLTextureInfo uvTextureInfo = {CVOpenGLESTextureGetTarget(uv_texture_ref_), CVOpenGLESTextureGetName(uv_texture_ref_), GR_GL_RGBA8};
    GrBackendTexture uvBackendTexture(bounds.width(), bounds.height(), GrMipMapped::kNo, uvTextureInfo);
    ...
    SkYUVAIndex yuvaIndices[4] = {
            ...
            SkYUVAIndex{1, SkColorChannel::kA},// Read V data from the alpha channel of the second texture, normal NV12 data V should be taken from the green channel, but currently only the uv texture created by GL_LUMINANCE_ALPHA can be used, so the V value is taken from the alpha channel
    ...
    sk_sp<SkImage> image = SkImage::MakeFromYUVATextures(context,
...
bool IOSExternalTextureGL::IsTexturesAvailable() {
    return ((pixelFormat == kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange  || pixelFormat == kCVPixelFormatType_420YpCbCr8BiPlanarFullRange) && (y_texture_ref_ && uv_texture_ref_ )) || (pixelFormat == kCVPixelFormatType_32BGRA && texture_ref_);
    // END
}
